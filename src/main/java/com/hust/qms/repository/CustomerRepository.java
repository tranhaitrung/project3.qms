...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(value = "select * from customers where (:search is null or LOCATE(:search, CONCAT_WS(', ', full_name, username, email))) AND (:status is null or status = :status)", nativeQuery = true)
    Page<Customer> listCustomer(@Param("search") String search, @Param("status") String status, Pageable pageable);
